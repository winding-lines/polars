...
pub fn is_object_store_url<P: AsRef<Path>>(p: P) -> bool {
    let path = p.as_ref();
    println!("polars_io/lib.rs | is_object_store_url {:?}\n", p.as_ref());
    return CloudType::from_str(&path.to_string_lossy())
        .map_err(|e| {
            println!("polars_io/lib.rs | got error {:?}", e);
            e
        })
        .is_ok();
}
